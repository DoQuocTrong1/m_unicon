...
import { Text, ListItem, View, Right } from 'native-base';
...
const Sidebar = ({ ...props }) => (<ScrollView>
    ...
    color: '#d82c1e',
        ...
        <FlatList data={routes} renderItem={({ item }) => (<ListItem style={[style.menuItem,
    ...
    item.route === 'Logout' // eslint-disable-line
        ? props.logOut() : props.navigation.navigate(item.route);
                    ...
                    <item.fontIcon size={24} style={style.menuIconLeft} name={item.icon}/>
                    <Text style={{ textAlign: 'left' }}>{item.caption}</Text>
                    <Right style={{ position: 'absolute', right: 23 }}>
                        <FontAwesome5 size={8} color={props.screen === item.route ? '#e74c3c' : '#a5a5a5'} name="circle"/>
                    </Right>
                </ListItem>)}/>
    </ScrollView>);
