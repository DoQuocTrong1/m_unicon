...
axios.interceptors.response.use(null, async (error) => {
    if (error.config && error.response && error.response.status === 405) {
        ...
        await Keychain.setGenericPassword(REFRESH_TOKEN, responseRefreshToken.data.refreshToken);
        return await axios.request(error.config);
    }
    return Promise.reject(error);
});

const callApi = (endpoint, method, data, token, schema) => {
...
export default () => (next) => (action) => {
    ...
    return callApi(endpoint, method, data, token, schema).then(response => next(actionWith({
        success: response,
        type: successType,
